...
export const ManageableContentComponent = () => {
    ...
    const [manageable, setManageable] = useState<IManageableSaturated|null>(null)
            ...
            <div className={css.contentWrapper}>
                <div className={css.content}>
                    ...
                    <div className={css.linkedWithContainer}>
                        <h3 className={
                            classNames(css.title, css.centeredText, css.borderBottom, css.m)}>Связи</h3>
                        <div className={css.linksTitles}>
                            <h3 className={classNames(css.title, css.linksTitle, css.centeredText, css.borderRight)}>Пользователи</h3>
                            <h3 className={classNames(css.title, css.linksTitle, css.centeredText)}>Отделы</h3>
                        </div>
                        <div className={css.rowContainer}>
                            <div className={classNames(css.halfWidthContainer, css.borderRight)}>
                                {manageable.user_ref.map(u => <ManageableLink
                                    link={`/user/${u.id}`}
                                    title={`${u.full_name?.replace('None', '')} | ${u.commentary}`}
                                    key={u.id}/>)}
                            </div>
                            <div className={css.halfWidthContainer}>
                                {manageable.departments.map(d => <ManageableLink
