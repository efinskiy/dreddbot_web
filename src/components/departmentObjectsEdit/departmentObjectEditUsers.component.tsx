...
export const DepartmentObjectEditUsersComponent = () => {
    const { id } = useParams();
    const [usersInSelectAvailable, setUsersInSelectAvailable] = useState<
        IUserInSelect[]
    >([]);
    const [usersInSelectSelected, setUsersInSelectSelected] = useState<
        IUserInSelect[]
    >([]);
    const [usersInSelectSelectedInitial, setUsersInSelectSelectedInitial] =
        useState<IUserInSelect[]>([]);
    ...
    const updateUsers = () => {
        const data = figureChanges(
            usersInSelectSelectedInitial,
            usersInSelectSelected
        );
        console.log(data);
        UpdateDepartmentUsers(Number(id), data).then((d) => {
            updateCfData(d, useSystem);
    ...
    return (
        <div className={css.block}>
            <MultiSelect
                ...
                onChange={(d: IUserInSelect[]) => {
                    setUsersInSelectSelected(d);
                }}
                showSearch={true}
                showSelectAll={false}
            />
